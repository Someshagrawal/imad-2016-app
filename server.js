...
app.get('/blog/:input', function (req, res) {
 ...
 if(true/*req.session && req.session.auth && req.session.auth.userId*/)
  ...
  blogfull = blogfull + '<br><br><br><h2><li><span class="heading">' + result.rows[i].Title + '</span></li></h2><p>' + result.rows[i].Article +'</p><h3>Comments</h3>' + result.rows[i].comment + '<h4>Your Comment:</h4><input id="T'+result.rows[i].Sno+'"  type="text"/> &emsp; <button id="B'+ result.rows[i].Sno+'">SUBMIT</button>';
     }
